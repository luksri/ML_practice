...
def create_chat_graph():
    ...
    def calendar(state: State) ->dict:
        ...
        option = st.selectbox("Choose an option:", dates, key="selectbox")
        print("i am selecting an option in dates")
        if st.button("Submit Choice", key="select date"):
            st.session_state["date_decision"] = option
        
        user_input = input("select 'y' to proceed")
        if user_input == 'y':
            pass
    ...
    def human_approval(state: State) -> Command[Literal["calendar", END]]:
        # interrupt.after()  # Pause graph execution until user input is received
        # user_decision = st.session_state.get("user_decision", "None")
        if st.button('Yes'):
            st.session_state['user_decision'] = 'yes'
        elif st.button('No'):
            st.session_state['user_decision'] = 'no'
...
if user_input:
    ...
    # st.session_state.chat_graph.invoke(Command(resume=st.session_state.user_decision), config=config)
    ...
    #     # Display choices in radio buttons
    #     # selected_option = st.radio("Select an option:", st.session_state["date_decision"])
    #     option = st.selectbox("Choose an option:", st.session_state["date_decision"], key="selectbox")
    ...
    print(f"st.session_state.date_decision - {st.session_state.date_decision}")
    if st.session_state.date_decision:
        ...
        if st.button("Submit Choice", key="select site"):
            st.session_state["site_decision"] = selected_option

    print(f"st.session_state.site_decision - {st.session_state.site_decision}")
