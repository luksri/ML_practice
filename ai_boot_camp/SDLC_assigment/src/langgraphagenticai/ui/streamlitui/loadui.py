    ...
    def load_streamlit_ui(self):
        ...
        with st.sidebar:
            ...
            if st.session_state.workflow_step == 0:
                if st.button("Start: Add Requirements"):
            ...
            if st.session_state.workflow_step > 0:
                ...
                if st.session_state.workflow_step == 1:
                    next_step_name = "user stories"
                elif st.session_state.workflow_step == 2:
                    next_step_name = "Design documents"
                elif st.session_state.workflow_step == 3:
                    next_step_name = "code"
                    
                if st.button(f"Next: {next_step_name}"):
                    st.session_state.workflow_step += 1
                    if st.session_state.workflow_step > 3:  # Reset after last step
                        st.session_state.workflow_step = 0
                    st.rerun()
            # # Use case selection
